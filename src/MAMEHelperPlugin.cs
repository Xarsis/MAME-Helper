...
namespace MAMEHelper
{
    public class MAMEHelperPlugin : GenericPlugin
    {
        ...
        private MAMEHelperSettingsViewModel _settingsViewModel;
        ...
        public MAMEHelperSettings LoadSettings()
        {
            try
            {
                ...
                if (File.Exists(path))
                    return JsonConvert.DeserializeObject<MAMEHelperSettings>(File.ReadAllText(path));
            }
            catch (Exception ex)
            {
            ...
            GetMainMenuItemsArgs menuArgs)
        {
            ...
            yield return MenuItem(tagMenu, "Set Category…",
                _ => WithRomData(d =>
                {
                    string val = PromptString("Enter the Category name to apply to all matched MAME games:", "Arcade");
            ...
            yield return MenuItem(tagMenu, "Set Source…",
                _ => WithRomData(d =>
                {
                    string val = PromptString("Enter the Source name to apply to all matched MAME games:", "MAME");
            ...
            yield return MenuItem(tagMenu, "Set Platform…",
                _ => WithRomData(d =>
                {
                    string val = PromptString("Enter the Platform name to apply to all matched MAME games:", "Arcade");
            ...
            yield return MenuItem(hideMenu, "Hide by Year Range…",
                _ => WithRomData(d =>
                {
                    string from = PromptString("Hide games BEFORE this year (e.g. 1985):", "1985");
                    if (from == null) return;
                    string to   = PromptString("Hide games AFTER this year (e.g. 1995):", "1995");
                    if (to == null) return;

                    if (int.TryParse(from, out int fromYear) && int.TryParse(to, out int toYear))
                        _hider.HideByYearRange(d, fromYear, toYear);
                    else
                        PlayniteApi.Dialogs.ShowErrorMessage("Please enter valid 4-digit years.", "MAME Helper");
                }));

            yield return MenuItem(hideMenu, "Hide by Manufacturer…",
                _ => WithRomData(d =>
                {
                    string mfr = PromptString("Enter manufacturer name to hide (partial match):", "");
            ...
            yield return MenuItem(hideMenu, "Unhide All MAME Games",
                _ =>
                {
                    if (Confirm("This will un-hide all currently hidden games in your library.\nContinue?"))
                        _hider.UnhideAll();
                });
            ...
            yield return MenuItem(mediaMenu, "Set Cover Images from Folder",
                _ => WithRomData(d =>
                    _media.SetCoverImages(d, _settings.CoverImageFolder)));

            yield return MenuItem(mediaMenu, "Set Background Images from Folder",
                _ => WithRomData(d =>
                    _media.SetBackgroundImages(d, _settings.BackgroundImageFolder)));
        ...
        /// <summary>
        /// Loads ROM data then runs the action. Aborts cleanly if data is unavailable.
        /// </summary>
        private void WithRomData(Action<Dictionary<string, RomsetMachine>> action)
        {
            try
            {
                ...
                {
                    // GetRomData already showed an error message.
                    return;
                }
                action(data);
            }
        ...
        /// <summary>Shows a Yes/No confirmation dialog. Returns true if the user clicks Yes.</summary>
        ...
        private static MainMenuItem MenuItem(string section, string description,
            Action<MainMenuItemActionArgs> action)
        {
        ...
        /// <summary>
        /// Creates a visual separator in the menu.
        /// Playnite uses a Description of "---" for separators (or "-").
        /// </summary>
